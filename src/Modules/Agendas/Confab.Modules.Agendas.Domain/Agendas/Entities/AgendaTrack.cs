...
public sealed class AgendaTrack : AggregateRoot
{
    ...
    // Only RegularAgendaSlots have AgendaItems
    internal void ChangeSlotAgendaItem(EntityId id, AgendaItem agendaItemId) // It's internal, because can be only used by domain service
    {
        var slot = _slots.FirstOrDefault(s => s.Id == id);
